...
rule malware_PSKiller_sys
{
	...
	condition:
		( uint16(0)==0x5A4D) and ( filesize <1MB) and pe.imports("ntoskrnl.exe","PsGetProcessId") and pe.imports("ntoskrnl.exe","PsLookupProcessByProcessId") and pe.imports("ntoskrnl.exe","PsGetProcessImageFileName") and pe.imports("ntoskrnl.exe","_stricmp") and pe.imports("ntoskrnl.exe","ZwTerminateProcess") and pe.imports("ntoskrnl.exe","ZwClose") and (pe.subsystem==pe.SUBSYSTEM_NATIVE) and (3 of ($str*))
}
